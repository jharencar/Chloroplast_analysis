...
plink2 --vcf chloroplast_w.outgroups_filtered.vcf.gz --recode transpose --out $PREFIX --allow-extra-chr

## Run PCA with bim/bam input
plink2 --tfile ${PREFIX} --pca --out ${PREFIX} --allow-extra-chr

#NOTES: not working... added a missingness filter that removed 75 samples (--mind 0.1) - still not working; PICKUP retry on the file with just alle and vill and hybs, work on tree with larger dataset
```